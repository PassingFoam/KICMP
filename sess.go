...
type (
	// ICMPSession defines a KCP session implemented by UDP
	ICMPSession struct {
		id      int
		seq     int
...
// newICMPSession create a new udp session for client or server
func newICMPSession(conv uint32, dataShards, parityShards int, l *Listener, conn net.PacketConn, ownConn bool, remote net.Addr, block BlockCrypt, icmproto ipv4.ICMPType) *ICMPSession {
	sess := new(ICMPSession)
	...
	//if _, ok := conn.(*icmp.PacketConn); ok {
	//	addr, err := net.ResolveIPAddr("ip", conn.LocalAddr().String())
	//	if err == nil {
	//		if addr.IP.To4() != nil {
	//			sess.xconn = conn.(*icmp.PacketConn).IPv4PacketConn()
	//		} else {
	//			sess.xconn = conn.(*icmp.PacketConn).IPv6PacketConn()
	//		}
	//	}
	//}
...
func (s *ICMPSession) Read(b []byte) (n int, err error) {
	for {
		s.mu.Lock()
...
func (s *ICMPSession) Write(b []byte) (n int, err error) { return s.WriteBuffers([][]byte{b}) }

// WriteBuffers write a vector of byte slices to the underlying connection
func (s *ICMPSession) WriteBuffers(v [][]byte) (n int, err error) {
	for {
		select {
...
func (s *ICMPSession) LocalAddr() net.Addr { return s.conn.LocalAddr() }

// RemoteAddr returns the remote network address. The Addr returned is shared by all invocations of RemoteAddr, so do not modify it.
func (s *ICMPSession) RemoteAddr() net.Addr { return s.remote }
...
func (s *ICMPSession) SetWriteDelay(delay bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.writeDelay = delay
}
...
func (s *ICMPSession) SetWindowSize(sndwnd, rcvwnd int) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ICMPSession) SetMtu(mtu int) bool {
	if mtu > mtuLimit {
		return false
	}
...
func (s *ICMPSession) SetStreamMode(enable bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if enable {
		s.kcp.stream = 1
...
func (s *ICMPSession) SetACKNoDelay(nodelay bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ICMPSession) SetDUP(dup int) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.dup = dup
}
...
func (s *ICMPSession) SetNoDelay(nodelay, interval, resend, nc int) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ICMPSession) SetDSCP(dscp int) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.l != nil {
...
func (s *ICMPSession) SetReadBuffer(bytes int) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.l == nil {
...
func (s *ICMPSession) SetWriteBuffer(bytes int) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.l == nil {
...
// 3. Encryption
// 4. TxQueue
func (s *ICMPSession) output(buf []byte) {
	var ecc [][]byte
...
func (s *ICMPSession) GetConv() uint32 { return s.kcp.conv }

// GetRTO gets current rto of the session
func (s *ICMPSession) GetRTO() uint32 {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.kcp.rx_rto
}
...
func (s *ICMPSession) GetSRTT() int32 {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *ICMPSession) GetSRTTVar() int32 {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.kcp.rx_rttvar
}

func (s *ICMPSession) notifyReadEvent() {
	select {
...
func (s *ICMPSession) notifyWriteEvent() {
	select {
...
func (s *ICMPSession) notifyReadError(err error) {
...
func (s *ICMPSession) notifyWriteError(err error) {
...
// packet input stage
func (s *ICMPSession) packetInput(data []byte) {
...
func (s *ICMPSession) kcpInput(data []byte) {
...
type (
	...
	Listener struct {
		...
		sessions        map[string]*ICMPSession // all sessions accepted by this Listener
		sessionLock     sync.RWMutex
		chAccepts       chan *ICMPSession // Listen() backlog
...
func (l *Listener) AcceptKCP() (*ICMPSession, error) {
...
func ListenWithOptions(laddr string, block BlockCrypt, dataShards, parityShards int) (*Listener, error) {
	//icmpaddr, err := net.ResolveIPAddr("ip", laddr)
	//if err != nil {
	//	return nil, errors.WithStack(err)
	//}
	icmpaddr := strings.Split(laddr, ":")[0]
	conn, err := icmp.ListenPacket("ip4:icmp", icmpaddr)
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func serveConn(block BlockCrypt, dataShards, parityShards int, conn net.PacketConn, ownConn bool) (*Listener, error) {
	...
	l.conn = conn
	l.ownConn = ownConn
	l.sessions = make(map[string]*ICMPSession)
	l.chAccepts = make(chan *ICMPSession, acceptBacklog)
...
//
// Check https://github.com/klauspost/reedsolomon for details
// Dial初始化过程，block为加密算法，默认为你不加密及nil，dataShards,parityShard有关奇偶校验包
func DialWithOptions(raddr string, block BlockCrypt, dataShards, parityShards int) (*ICMPSession, error) {
	// network type detection
	icmpaddr, err := net.ResolveIPAddr("ip", raddr)
	if err != nil {
		return nil, errors.WithStack(err)
	}
	network := "ip4:icmp"
...
func NewConn2(raddr net.Addr, block BlockCrypt, dataShards, parityShards int, conn net.PacketConn) (*ICMPSession, error) {
	var convid uint32
	binary.Read(rand.Reader, binary.LittleEndian, &convid)
...
func NewConn(raddr string, block BlockCrypt, dataShards, parityShards int, conn net.PacketConn) (*ICMPSession, error) {
	icmpaddr, err := net.ResolveIPAddr("ip", raddr)
	if err != nil {
		return nil, errors.WithStack(err)
	}
	return NewConn2(icmpaddr, block, dataShards, parityShards, conn)
}
